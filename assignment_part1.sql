...
('Russia'			,143	 ,	'Moscow'),
('Mexico'			,128	 ,	'MexicoCity'),
('Japan'			,126	 ,	'Tokyo'),
('Philippines'	,	102	 ,	'Manila'),
('Ethiopia'		,101	 ,	'AddisAbaba'),
('Vietnam' 		,94	 ,	'Hanoi'),
('Egypt'			,93	 ,	'Cairo'),
('Germany'		,81	 ,	'Berlin'),
('Iran'			,80	 ,	'Tehran'),
...
('Congo'			,79	 ,	'Kinshasa'),
('France'			,64	 ,	'Paris'),
('UnitedKingdom'	,65	 ,	'London'),
('Italy'			,60	 ,	'Rome'),
('SouthAfrica'	,	55	, 	'Pretoria'),
('Myanmar'		,54	 	,'Naypyidaw');


select * from countries;

insert into countries values('portugal',100,'lisbon'),('saudi arabia',30,'riyadh');

update countries set capital='newdelhi' where name='india';

alter table countries rename to big_countries;

select * from big_countries;

create table product(product_id int primary key auto_increment,product_name varchar(50) not null unique,description varchar(300),sup_id int,foreign key(sup_id) references suppliers(supplier_id) on delete cascade on update cascade);


create table suppliers(supplier_id int primaRy key auto_increment,supplier_name varchar(50),location varchar(40));

create table stock(id int primary key auto_increment,p_id int ,balance_stock integer,foreign key(p_id) references product(product_id) on delete cascade on update cascade);
...
insert into suppliers values(20,'apple','usa'),(30,'samsung','korea');
insert into product values(1,'iphone','it is an apple product',20);
insert into stock values(100,1,5000);
...
alter table suppliers modify supplier_name varchar(50) not null unique;
...
alter table emp modify emp_no integer unique;
desc emp;


/* emp_sal is the view created */
select * from emp_sal;
